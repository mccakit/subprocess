...
export module subprocess:shell_utils;

import std;
export import :shell_utils_fwd;
import :basic_types;
...
namespace subprocess
{
    ...
    {
        const char *ptr = std::getenv(var.c_str());
        if (ptr == nullptr)
            return "";
        return ptr;
    }
    ...
    {
#ifdef _WIN32
        std::string path_ext = subprocess::getenv("PATHEXT");
        if (path_ext.empty())
            path_ext = "exe";
        if (is_file(path))
            return path;
    ...
    static std::string find_program_in_path(const std::string &name)
    {
        ...
        std::string path_env = subprocess::getenv("PATH");
    ...
    static bool is_python3(std::string path)
    {
        CompletedProcess process =
            subprocess::run({path, "--version"}, RunBuilder().cout(PipeOption::pipe).cerr(PipeOption::cout));
